...
First, I will focus on the Principal Components GARCH model. The Principal Components GARCH model is defined as follows:
\begin{gather}
    y_t = H_t^{\frac{1}{2}}\varepsilon_t\\
    H_t=P\Lambda_tP^{\top}
\end{gather}
Here, $P$ is a $m\times m$ orthogonal matrix while $\Lambda_t$ is a $m\times m$ diagonal matrix with the diagonal elements being denoted by $\text{diag}(\lambda_{1,t},\lambda_{2,t},...\lambda_{m,t})$. The orthogonal matrix is typically estimated by Principal Component Analysis while the elements of $\Lambda_t$ are assumed to follow an univariate GARCH(1,1) model.
\begin{gather}
    \lambda_{i,t}=\delta_i+\alpha_i y_{i,t-1}^{(2)}+\beta_i
\end{gather}
...
Unlike in the scalar version of the GARCH model proving geometric ergodicity and $\beta$-mixing for the Functional GARCH model is very difficult. That is because the concept of Since (weak) ergodicity is not sufficient for the application of the functional central limit theorem, \citet{Aue2017} derive the following result.
\begin{corollary}
    \begin{enumerate}[label=(\Roman*)]
        \item
