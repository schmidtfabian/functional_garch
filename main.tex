...
\end{align}
From this representation, it can be seen that the volatility equation of the strong GARCH process can also be represented as an AR model with a random coefficient. This representation is useful to study the existence of a stationary and ergodic solution.\\
...
\end{theorem}
To prove \ref{stationarityscalarGARCH(1,1)}, note that Equation (\ref{Eq2StrongGARCH}) can be solved recursively to attain $h_t$. Showing that $h_t$ converges involves an application of the Cauchy Root Test as well as the strong law of large numbers. Uniqueness then follows by contradiction, assuming the existence of another solution and showing that it must coincide with the original solution. Ergodicity follows from the fact that the volatility equation of the GARCH(1,1) model can be stated as a homogeneous Markov Chain that is irreducible, aperiodic, and positive recurrent. It can actually be shown that the GARCH(1,1) process is geometrically ergodic under mild assumptions. Ergodicity describes the  The complete proof can be found in the Appendix.\\ % Add a sentence on ergodicity and non-anticipativeness Maybe try using Cleveref here if there is time.
...
\begin{align}
    ...
    \end{pmatrix}
\end{align}
To estimate $d$, $A$ and $B$ it is also necessary to project $(y_i(t)^2)_{i\in[1,N]}$ and $(\sigma_i(t))_{i\in[1,N]}$ onto $\Phi_M$ and define the m-dimensional vectors $\mathbf{y}_i^{(2)}=\begin{pmatrix}
    y_{i,1}^{(2)} & \ldots & y_{i,M}^{(2)}
\end{pmatrix}^\top$ and $\mathbf{s}_i^{(2)}=\begin{pmatrix}
    s_{i,1}^{(2)} & \ldots & s_{i,M}^{(2)}
\end{pmatrix}\top$ where their elements are the result of the inner product with the elements of $\Phi_M$ $y_{i,m}^{(2)}=\langle y_i(t),\varphi_m(t)\rangle$
\begin{align}
